...
class ViewController: UIViewController {
    ...
    @IBAction func synchronousDownload(_ sender: UIBarButtonItem) {
        ...
        let url = URL(fileURLWithPath: BigImages.seaLion.rawValue)
    ...
    @IBAction func simpleAsynchronousDownload(_ sender: UIBarButtonItem) {
        let url = URL(fileURLWithPath: BigImages.shark.rawValue)
        let download = DispatchQueue(label: "downloadSharkImage")
        download.async {
            let imgData = try? Data(contentsOf: url)
            ...
            if imgData != nil {
                ...
                // Display it
                //Always run UIKit on main
                //If the names ends view, use the main queue
                DispatchQueue.main.async {
                    self.photoView.image = image
                }
            } else {
                print("Got nil, damm shark")
            }
        }
    ...
    func withBigImage(completionHandler handler: @escaping (_ image: UIImage) -> Void){
        ...
        DispatchQueue.global(qos: .userInitiated).async { () -> Void in
            ...
            if let url = URL(string: BigImages.whale.rawValue), let imgData = try? Data(contentsOf: url), let img = UIImage(data: imgData) {
                ...
                print("Something returned nil", try? Data(contentsOf: URL(fileURLWithPath: BigImages.whale.rawValue)))
            }
        }
    }

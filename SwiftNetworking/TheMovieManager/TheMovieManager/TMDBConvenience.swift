...
extension TMDBClient {
    ...
    */
    ...
    private func getRequestToken(_ completionHandlerForToken: @escaping (_ success: Bool, _ requestToken: String?, _ errorString: String?) -> Void) {
        ...
        _ = taskForGETMethod(Methods.AuthenticationTokenNew, parameters: parameters) { (result, error) in
            
            if let error = error {
                print(error)
                completionHandlerForToken(false, nil, "Login Error, Request Token failed.")
            } else {
                ...
                if let requestToken = result?[TMDBClient.JSONResponseKeys.RequestToken] as? String {
                    ...
                    completionHandlerForToken(false, nil, "Can't get Request token")
                }
            }
        }
    ...
    private func getSessionID(_ requestToken: String?, completionHandlerForSession: @escaping (_ success: Bool, _ sessionID: String?, _ errorString: String?) -> Void) {
        ...
        parameters[TMDBClient.ParameterKeys.RequestToken] = requestToken! as AnyObject?
        
        _ = taskForGETMethod(Methods.AuthenticationSessionNew, parameters: parameters, completionHandlerForGET: { (result, error) in
            
            if let error = error{
                print(error)
                completionHandlerForSession(false, nil, "Error on requesting session")
            } else {
                
                if let session = result?[TMDBClient.JSONResponseKeys.SessionID] as? String {
                    print("Session is good", result?[TMDBClient.JSONResponseKeys.SessionID] as? String ?? "nil" )
                    completionHandlerForSession(true, session, nil)
                }else {
    ...
    private func getUserID(_ completionHandlerForUserID: @escaping (_ success: Bool, _ userID: Int?, _ errorString: String?) -> Void) {
        ...
        let parameters = [TMDBClient.ParameterKeys.SessionID: TMDBClient.sharedInstance().sessionID! as AnyObject] as [String : AnyObject]
        
        _ = taskForGETMethod(Methods.Account, parameters: parameters, completionHandlerForGET: { (result, error) in
            
            
            if let error = error {
                print(error)
                completionHandlerForUserID(false, nil, "Error Fetching User ID")
            } else {
                
                if let userID = result?[TMDBClient.JSONResponseKeys.UserID] as? Int {
                    completionHandlerForUserID(true, userID, nil)
                } else {
                    print("Error Fetching User iD With key \( TMDBClient.JSONResponseKeys.UserID)")
                    completionHandlerForUserID(false, nil, "Error Fetching User ID With key \( TMDBClient.JSONResponseKeys.UserID)")
                }
    ...
    func getWatchlistMovies(_ completionHandlerForWatchlist: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        _ = taskForGETMethod(mutableMethod, parameters: parameters as [String : AnyObject], completionHandlerForGET: { (results, error) in
            ...
            if let error = error {
                ...
                if let watchListMovies = results?[TMDBClient.JSONResponseKeys.MovieResults] as? [[String:AnyObject]] {
                    let movies = TMDBMovie.moviesFromResults(watchListMovies)
                    completionHandlerForWatchlist(movies, nil)
                } else {
                    completionHandlerForWatchlist(nil, NSError(domain: "getWatchlist parsgin", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse getWatchlistMovies"]))
                }
    ...
    func getMoviesForSearchString(_ searchString: String, completionHandlerForMovies: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) -> URLSessionDataTask? {
        ...
        _ = taskForGETMethod(Methods.SearchMovie, parameters: parameters as [String:AnyObject], completionHandlerForGET: { (results, error) in
            if let error = error {
    ...
    func postToWatchlist(_ movie: TMDBMovie, watchlist: Bool, completionHandlerForWatchlist: @escaping (_ result: Int?, _ error: NSError?) -> Void) {
        ...
        _ = taskForPOSTMethod(mutableMethod, parameters: parameters as [String:AnyObject], jsonBody: jsonBody, completionHandlerForPOST: { (results, error) in
            
            if let error = error {
                completionHandlerForWatchlist(nil, error)
            } else {
                
                if let responseCode = results?[TMDBClient.JSONResponseKeys.StatusCode] as? Int {
                    completionHandlerForWatchlist(responseCode, nil)
                } else {
